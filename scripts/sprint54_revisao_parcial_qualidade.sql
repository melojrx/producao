-- ============================================================
-- SPRINT 54.6 — Revisao parcial de Qualidade
-- A revisao pode ser parcial; somente aprovadas consomem a pendencia.
...
DECLARE
  v_saldo RECORD;
BEGIN
  IF NEW.turno_setor_operacao_id_qualidade IS NULL THEN
    RETURN NEW;
  END IF;
  ...
  IF COALESCE(NEW.quantidade_revisada, 0) > COALESCE(v_saldo.saldo_fisico, 0) THEN
    ...
    RAISE EXCEPTION 'A OP % possui apenas % peça(s) com saldo físico nesta operação. Ajuste a quantidade revisada ou selecione outra OP.', v_saldo.numero_op, v_saldo.saldo_fisico
      USING ERRCODE = 'P0001';
  END IF;
...
DECLARE
  ...
  v_quantidade_aprovada_acumulada INTEGER;
  v_saldo_pendente_operacao INTEGER;
...
BEGIN
  ...
  v_saldo_pendente_operacao := GREATEST(
    v_operacao.quantidade_planejada - v_operacao.quantidade_realizada,
    0
  );

  IF v_quantidade_revisada > v_saldo_pendente_operacao THEN
    RAISE EXCEPTION 'A quantidade revisada não pode ultrapassar a pendência disponível.'
      USING ERRCODE = 'P0001';
  END IF;
  ...
  v_quantidade_aprovada_acumulada := v_operacao.quantidade_realizada + p_quantidade_aprovada;
  v_status_operacao := CASE
    WHEN v_quantidade_aprovada_acumulada >= v_operacao.quantidade_planejada THEN 'concluida'
    ELSE 'em_andamento'
  END;
  ...
  SET
    quantidade_realizada = v_quantidade_aprovada_acumulada,
  ...
  SELECT
    ...
    v_quantidade_revisada,
    v_total_defeitos,
    v_quantidade_aprovada_acumulada,
    GREATEST(v_operacao.quantidade_planejada - v_quantidade_aprovada_acumulada, 0),
    v_status_operacao,
