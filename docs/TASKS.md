...
- [x] **HU 51.6 — Como revisor, quero registrar múltiplos defeitos por operação no mesmo lote, para refletir a realidade da análise de qualidade sem limitar defeito à quantidade de peças reprovadas.**
  **Prioridade:** P0
  **Risco:** Alto
  ...
  **Evidência:** removida a regra antiga que exigia `sum(quantidade_defeito) = quantidade_reprovada` em `lib/utils/qualidade-lotes.ts`, `components/apontamentos/PainelQualidadeSupervisor.tsx` e na RPC `registrar_revisao_lote_qualidade` do script `scripts/sprint51_fluxo_continuo_qualidade.sql`. O painel administrativo passou a orientar o revisor a registrar ocorrências de defeito, permitindo que uma mesma peça concentre múltiplos defeitos. Criado teste automatizado para o caso `1` reprovada e `4` ocorrências de defeito, validando que a revisão do lote permanece permitida. SQL idempotente reaplicado no Supabase remoto `jsuufbgdcqxogimmocof` em `2026-05-18`; `scripts/sprint51_validate_lote_review.mjs` revisou um lote temporário com `quantidade_reprovada = 1`, `total_defeitos = 4`, `detalhes = 2`, `detalhes_catalogados = 2`, `status_lote = revisado`, e removeu todos os dados temporários (`detalhes_removidos = 2`, `registros_qualidade_removidos = 1`, `lotes_removidos = 1`, `registros_producao_removidos = 1`). Como a alteração atua apenas na validação e na gravação de `qualidade_registros`/`qualidade_detalhes`, sem escrita em consolidados operacionais, os KPIs produtivos permanecem fora do cálculo de defeitos. Validação local em `2026-05-18`: `node --test --experimental-strip-types lib/utils/qualidade-lotes.test.ts lib/utils/qualidade-indicadores.test.ts` passou 11/11; `npx tsc --noEmit` e `git diff --check` passaram sem erros.

- [x] **HU 51.7 — Como revisor, quero que o input de revisão tenha Operação, Tipo de Defeito, Quantidade e Observação, para registrar defeitos com contexto operacional completo.**
  **Prioridade:** P0
  **Risco:** Alto
  ...
  **Evidência:** o formulário de revisão de lote em `components/apontamentos/PainelQualidadeSupervisor.tsx` passou a exibir, em cada linha de defeito, os campos `Operação`, `Tipo de defeito`, `Qtd.` e `Observação opcional`. A operação nasce pré-selecionada com a operação produtiva de origem do lote e pode ser corrigida para outra operação produtiva da mesma OP; operações de setor com `modo_apontamento = revisao_qualidade` ficam fora das opções. `lib/actions/qualidade.ts` passou a enviar `turno_setor_operacao_id_origem` em cada detalhe, e a RPC `registrar_revisao_lote_qualidade` em `scripts/sprint51_fluxo_continuo_qualidade.sql` passou a validar que a operação pertence à mesma OP do lote e é produtiva antes de gravar `qualidade_detalhes` com a operação, setor, tipo de defeito, quantidade e observação informados na linha. Criado teste automatizado garantindo que defeito sem operação produtiva explícita é recusado. SQL idempotente reaplicado no Supabase remoto `jsuufbgdcqxogimmocof` em `2026-05-18`; `scripts/sprint51_validate_lote_review.mjs` revisou lote temporário com duas linhas catalogadas, ambas com `turno_setor_operacao_id_origem`, retornando `quantidade_reprovada = 1`, `total_defeitos = 4`, `operacoes_com_defeito = 1`, `detalhes = 2`, `detalhes_catalogados = 2`, `status_lote = revisado`, e limpou todos os dados temporários. Validação local em `2026-05-18`: `node --test --experimental-strip-types lib/utils/qualidade-lotes.test.ts lib/utils/qualidade-indicadores.test.ts` passou 12/12; `npx tsc --noEmit` passou sem erros.

- [x] **HU 51.8 — Como administrador, quero um CRUD completo de tipos de defeito, para padronizar a coleta e gerar KPIs confiáveis de qualidade.**
  **Prioridade:** P0
  **Risco:** Médio
...
- [x] **HU 51.9 — Como produto, quero remover totalmente o setor `Qualidade` legado do fluxo ativo, para que Qualidade opere apenas como fila contínua paralela por lotes.**
  ...
  **Evidência:** criado `lib/utils/qualidade.ts` com as regras `setorEhQualidadeLegado()` e `setorParticipaFluxoProdutivoAtivo()`, cobertas por `lib/utils/qualidade.test.ts`, para excluir do fluxo ativo qualquer setor chamado `Qualidade` ou com `modo_apontamento = revisao_qualidade`. `lib/queries/turnos.ts`, `lib/queries/turnos-client.ts` e `lib/queries/turno-setor-operacoes-base.ts` passaram a filtrar seções, setores, demandas e operações de Qualidade legado antes de hidratar dashboard, apontamentos e scanner. `lib/actions/turnos.ts` passou a exigir ao menos uma operação produtiva real no roteiro e a ignorar Qualidade legado no carry-over. O scanner bloqueia QR de setor Qualidade legado com orientação para revisar pela aba Qualidade dos apontamentos; `/admin/apontamentos` não renderiza mais o fallback antigo de seção de Qualidade e o painel de revisão opera somente por `qualidade_lotes`. Criado e aplicado `scripts/sprint51_remover_qualidade_legado_fluxo_ativo.sql`, que atualiza as funções `sincronizar_turno_setor_ops`, `sincronizar_turno_setor_operacoes` e `sincronizar_turno_setor_demanda_legada` para não derivar setor/QR/seção/operação/demanda de Qualidade legado em novos turnos. Validação remota em `2026-05-18` com `scripts/sprint51_validate_no_legacy_quality_sector.mjs` no Supabase `jsuufbgdcqxogimmocof`: produto temporário com roteiro contendo uma operação produtiva e uma operação de Qualidade legado criou `turno_ops_criadas = 1`, `secoes_produtivas = 1`, `secoes_qualidade = 0`, `operacoes_qualidade = 0`, `demandas_qualidade = 0` e `turno_setores_qualidade = 0`; a limpeza removeu todos os registros temporários. Validação local em `2026-05-18`: `node --test --experimental-strip-types lib/utils/qualidade.test.ts lib/utils/qualidade-defeitos.test.ts lib/utils/qualidade-lotes.test.ts lib/utils/qualidade-indicadores.test.ts` passou 19/19; `npx tsc --noEmit` passou sem erros; `npm run build` passou após rerun com permissão elevada por bloqueio de sandbox do Turbopack.

- [x] **HU 51.10 — Como supervisor, quero os KPIs de qualidade em uma aba própria da dashboard, para analisar qualidade sem misturar com a visão operacional de produção.**
  **Prioridade:** P1
  **Risco:** Médio
